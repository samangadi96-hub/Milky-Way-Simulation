...
void main() {
    ...
    // Scale from screen space to "world" space to match the ray marcher
    ...
    // Accretion disk squished by inclination and thickness
    ...
    if (r < shadow_radius) {
        ...
        if (disk_r > inner_r && disk_r < outer_r) {
            ...
            // Base Doppler scale (bright on left/one side, follows azimuth)
            ...
            float angle = atan(disk_p.y, disk_p.x) + u_time * 1.5; // rotates smoothly
            float cheap_noise = sin(angle * 4.0 + disk_r * 5.0) * 0.5 + 0.5; // very cheap approx
            ...
            // Apply a moderate doppler to this layer
    ...
    // Output premultiplied color for LOD fading and interpolated transmittance
    // to composite correctly over the 3D stars underneath.
